...
#error "You shall not pass, until you know what you want."
    ...
    struct ResolvedTrace : public Trace
    {
        ...
        struct SourceLoc
        {
            ...
            bool operator==(const SourceLoc& b) const { return function == b.function && filename == b.filename && line == b.line && col == b.col; }
    ...
    namespace details
    {
        ...
        private:
            ...
            static _Unwind_Reason_Code backtrace_trampoline(_Unwind_Context* ctx, void* self) { return (static_cast<Unwinder*>(self))->backtrace(ctx); }
    ...
    public:
        TraceResolverLinuxBase()
            : argv0_(get_argv0())
            , exec_path_(read_symlink("/proc/self/exe"))
        {
        }
    ...
    private:
        ...
        static void find_in_section_trampoline(bfd*, asection* section, void* data)
        {
            ...
            context->self->find_in_section(reinterpret_cast<bfd_vma>(context->addr), reinterpret_cast<bfd_vma>(context->base_addr), *context->fobj, section, context->result);
        }
...
#pragma clang diagnostic ignored "-Wzero-as-null-pointer-constant"
            ...
            {
                result.found = bfd_find_nearest_line(fobj.handle.get(), section, fobj.symtab.get(), addr - sec_addr, &result.filename, &result.funcname, &result.line);
            }

            if (!result.found && fobj.dynamic_symtab)
            {
                result.found = bfd_find_nearest_line(fobj.handle.get(), section, fobj.dynamic_symtab.get(), addr - sec_addr, &result.filename, &result.funcname, &result.line);
            }
#if defined(__clang__)
...
#define ELF_GET_DATA(ARCH)                                                                                                     \
    ...
    {                                                                                                                          \
        ...
        template <typename CB> static bool deep_first_search_by_pc(dwarf_fileobject& fobj, Dwarf_Die parent_die, Dwarf_Addr pc, std::vector<std::string>& ns, CB cb)
        {
            Dwarf_Die   current_die = 0;
            Dwarf_Debug dwarf       = fobj.dwarf_handle.get();
            Dwarf_Error error       = DW_DLE_NE;
    ...
    public:
        ...
        std::streamsize xsputn(const char_type* s, std::streamsize count) override { return static_cast<std::streamsize>(fwrite(s, sizeof *s, static_cast<size_t>(count), sink)); }
    ...
    private:
        void activate(ColorMode::type mode, int fd) { activate(mode == ColorMode::automatic && isatty(fd) ? ColorMode::always : mode); }
    ...
    private:
		...
		color(color &t) : m_color(t.m_color), m_console_handle(INVALID_HANDLE_VALUE) {}
		color& operator=(color) { return *this; }
        ...
        template<typename charT, typename traits> friend std::basic_ostream<charT, traits> & operator<<(std::basic_ostream<charT, traits> &lhs, color const &rhs);
    };
    ...
    public:
            ...
            , _enabled(false)
        {
            ...
            if (m_console_handle != INVALID_HANDLE_VALUE)
            {
				...
				m_reset_color = console_info.wAttributes;
			}
        }
		~Colorize()
		{
            if (m_console_handle != INVALID_HANDLE_VALUE)
            {
                SetConsoleTextAttribute(m_console_handle, m_reset_color);
            }
		}
		...
		{
			activate(mode != ColorMode::never ? ColorMode::always : ColorMode::never); 
		}
        void set_color(Color::type ccode)
        {
            if (!_enabled)
			...
			{
				if (ccode == Color::purple)
				{
					m_color = red;
				}
				else if (ccode == Color::yellow)
				{
					m_color = yellow;
				}
				...
				CONSOLE_SCREEN_BUFFER_INFO console_info;
				GetConsoleScreenBufferInfo(m_console_handle, &console_info);
				SetConsoleTextAttribute(m_console_handle, 0x0F & m_color | 0xf0 & console_info.wAttributes);
			}
